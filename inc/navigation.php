 ...
 * Forked from: https://github.com/roikles/Wordpress-Bem-Menu
 */

class walker_bem_nav extends Walker_Nav_Menu {
  function __construct($css_class_prefix) {
    ...
    $this->item_css_classes = array(
      ...
      'parent_of_active_item'     => 'is-active',
      'ancestor_of_active_item'   => 'is-active',
      'sub_menu'                  => $this->css_class_prefix . '-sub',
...
/**
 * navigation returns an instance of the walker_bem_nav class with the following arguments
...
function navigation ($location = "main_menu", $css_class_prefix = 'Nav') {
    $args = array(
      'theme_location'    => $location,
      'container'         => false,
      'items_wrap'        => '<ul class="' . $css_class_prefix . '-list">%3$s</ul>',
      'walker'            => new walker_bem_nav($css_class_prefix, true )
    );

    if (has_nav_menu($location )) {
      return wp_nav_menu($args);
    } else {
      "<p>You need to first define a menu in WP-admin<p>";
    }
}
